...
const mockMealsResponse = {
  meals: [
    ...
    },
    {
      ...
      mealType: 'lunch',
      items: [
        { food: 'Chicken', quantity: '200g', calories: 330, macros: { protein: 62, carbs: 0, fats: 7 } },
      ],
      createdAt: '',
      updatedAt: '',
    },
...
describe('DailyNutritionSummary', () => {
  ...
  it('renders card with date in title', async () => {
    vi.mocked(mealApi.getAll).mockResolvedValue({ meals: [] })
  ...
  it('displays total calories for the day', async () => {
    vi.mocked(mealApi.getAll).mockResolvedValue(mockMealsResponse)
    renderWithProvider(<DailyNutritionSummary date={targetDate} />)
